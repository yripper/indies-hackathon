...
async function handleIncomingMessageInner(
  ...
  const replayMessages = history.map((m, i) => {
    // Apply forwarding prefix to the last (current) user message so the LLM
    // sees the enriched version with forwarding context.
  ...
  try {
    ...
    // ALS carries the JID (for tool cache lookups), the ephemeral system note
    // (for agentNode to fold into the system prompt), the per-media DB recorder
    // closures, and the sendImage function. Errors in the DB inserts are
    // swallowed at the call site — a failed dashboard write must not break the
    // user-facing reply.
    const rawState = await Promise.race([
      requestContext.run(
